...
export function envsLoader({ envs, prefix, buildJsonPath }: EnvsLoaderOptions): Loader {
  ...
  const regex =
    /(_)(?:(?<vl_1_i>[0-9]+)_)?(?<vl_1>[A-Z0-9]+(?:_[A-Z0-9]+)*)(?:__(?:(?<vl_2_i>[0-9]*)_)?(?<vl_2>[A-Z0-9]+(?:_[A-Z0-9]+)*))?(?:__(?:(?<vl_3_i>[0-9]*)_)?(?<vl_3>[A-Z0-9]+(?:_[A-Z0-9]+)*))?(?:__(?:(?<vl_4_i>[0-9]*)_)?(?<vl_4>[A-Z0-9]+(?:_[A-Z0-9]+)*))?(?:__(?:(?<vl_5_i>[0-9]*)_)?(?<vl_5>[A-Z0-9]+(?:_[A-Z0-9]+)*))?/;
  return async function () {
    return toPairs(envs)
      .filter(([key]) => key.startsWith(prefix))
      .reduce((config, [str, value]) => {
        const match = regex.exec(str.slice(prefix.length));
        if (match) {
